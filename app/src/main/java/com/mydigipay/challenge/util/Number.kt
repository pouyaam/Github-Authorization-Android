...
fun Long.toHumanReadable(): String {
    if (this == Long.MIN_VALUE) return (Long.MIN_VALUE + 1).toHumanReadable()
    if (this < 0) return "-" + (-this).toHumanReadable()
...
fun Int.toHumanReadable() = this.toLong().toHumanReadable()